...
test('addInput', () => {
  ...
  expect(typeof addInput('input')).toEqual('function');
});
...
test('frames', () => {
  ...
  expect(typeof frames('options')).toEqual('function');
});
...
test('setBitrate', () => {
  ...
  expect(typeof setBitrate('a')).toEqual('function');
});
...
test('setCodec', () => {
  ...
  expect(typeof setCodec('a')).toEqual('function');
});
...
test('setMetaData', () => {
  ...
  expect(setMetaData([ [ 'flag', 'data' ], [ 'flag2', 'data2' ] ])).toEqual(' -metadata flag="data" -metadata flag2="data2"');
  expect(typeof setMetaData('a')).toEqual('function');
});

test('option strings', () => {
  expect.assertions(Object.keys(tests).length);
  Object.entries(tests)
  .forEach(([ k, { func, expected, input } ]) => expect(func(input)).toEqual(expected));
});
...
test('convertToVideo', () => {
  ...
  convertToVideo('demo.mp4', [ startTime(2), duration(4) ], `${outputFileName}`).then(x =>
    expect(x.spawnargs[ 2 ]).toEqual(`ffmpeg -i demo.mp4 ${startTime(2)}${duration(4)} ${outputFileName}`)
  );
});
